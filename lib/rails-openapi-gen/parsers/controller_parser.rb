...
class RailsOpenapiGen::Parsers::ControllerParser
  ...
  # Class method for batch processing multiple routes
  # @param routes [Array<Hash>] Array of route hashes
  # @param filter_missing [Boolean] Whether to filter out routes without templates (default: true)
  # @return [Array<Hash>] Array of controller info hashes with route included (filtered if requested)
  def self.parse_all(routes, filter_missing: true)
    logger = RailsOpenapiGen.logger
    logger.info("Parsing controller information for #{routes.size} routes...")
    
    controller_infos = routes.map do |route|
      parser = new(route)
      controller_info = parser.parse
      controller_info.merge(route: route)
    end

    if filter_missing
      valid_endpoints = controller_infos.select do |info|
        if info[:jbuilder_path]
          true
        else
          endpoint = "#{info[:route][:method]} #{info[:route][:path]}"
          logger.warn("No Jbuilder template found for #{endpoint}")
          false
        end
      end
      
      logger.info("Found #{valid_endpoints.size} endpoints with Jbuilder templates")
      valid_endpoints
    else
      controller_infos
    end
