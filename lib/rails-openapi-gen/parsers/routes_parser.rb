...
class RailsOpenapiGen::Parsers::RoutesParser
  ...
  # @param filter_config [Object, nil] Configuration object with route_included? method for filtering (nil = no filtering)
  def initialize(file_checker: File.method(:exist?), filter_config: nil)
  ...
  def parse
    ...
    # Apply filtering if configuration provided
    if @filter_config
      filtered_routes = routes.select do |route|
        @filter_config.route_included?(route[:path])
      end
