  ...
  describe '#extract_template_path' do
    context 'with render call without arguments' do
      ...
      it 'returns default jbuilder path' do
        ...
        action_node = ast.children.first
  ...
  describe 'JbuilderPathProcessor' do
    ...
    describe '#render_call?' do
      it 'identifies render calls correctly' do
        render_node = Parser::CurrentRuby.parse('render').children.first
        redirect_node = Parser::CurrentRuby.parse('redirect_to users_path').children.first
    ...
    describe '#extract_render_target' do
      context 'with empty arguments' do
        let(:render_node) { Parser::CurrentRuby.parse('render').children.first }
      ...
      context 'with string argument' do
        let(:render_node) { Parser::CurrentRuby.parse("render 'show'").children.first }
      ...
      context 'with hash options' do
        let(:render_node) { Parser::CurrentRuby.parse("render template: 'shared/user'").children.first }
    ...
    describe '#parse_render_options' do
      context 'with json option' do
        let(:hash_node) { Parser::CurrentRuby.parse("{ json: @users }").children.first }
      ...
      context 'with template option' do
        let(:hash_node) { Parser::CurrentRuby.parse("{ template: 'api/users/list' }").children.first }
    ...
    describe '#extract_render_hash_options' do
      let(:hash_node) { Parser::CurrentRuby.parse("{ template: 'shared/user', formats: :xml, status: :ok }").children.first }
    ...
    describe '#extract_node_value' do
      it 'extracts values from different node types' do
        str_node = Parser::CurrentRuby.parse('"test"').children.first
        sym_node = Parser::CurrentRuby.parse(':test').children.first
        true_node = Parser::CurrentRuby.parse('true').children.first
        false_node = Parser::CurrentRuby.parse('false').children.first
