...
RSpec.describe 'Example Use Cases Integration' do
  describe 'Orders endpoint with nested resources' do
    ...
    it 'generates correct schema for orders endpoint' do
      ...
      if ENV['DEBUG']
        puts "DEBUG: Item properties detail: #{item.properties.map do |p|
                                                 "#{p.property_name} (#{p.class.name})"
                                               end }"
      end
  ...
  describe 'Route inference for nested resources' do
    ...
    before do
      ...
      routes = [
        ...
        double('Route',
               verb: 'GET',
               path: double('Path', spec: '/api/v1/departments/employees/search(.:format)'),
               defaults: { controller: 'api/v1/departments/employees', action: 'search' },
               name: 'api_v1_departments_employees_search')
      ]
      ...
      allow(File).to receive(:exist?).with('/rails/app/app/controllers/api/v1/departments/employees_controller.rb').and_return(true)
    end
    ...
    it 'correctly infers nested controller paths' do
      ...
      employees_route = results.find { |r| r[:name] == 'api_v1_departments_employees_search' }
      expect(employees_route).not_to be_nil
      expect(employees_route[:controller]).to eq('api/v1/departments/employees')
    end
  end
